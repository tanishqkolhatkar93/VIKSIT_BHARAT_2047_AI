...
import { createPublicCard } from "../services/api";
import type { VisionCardData } from "../types/api";
import type { Dictionary } from "../utils/i18n";
import { ShareModal, type ShareMode } from "./ShareModal";
...
const APP_URL =
  ...
  (typeof window !== "undefined" ? window.location.origin : "viksitbharat2047.ai");
...
const THEMES: Record<string, Theme> = {
  education: {
    ...
    emoji: "🎓",
    accent: "#ffb43a",
    glow: "rgba(255, 180, 58, 0.32)",
    tag1: "EDUCATION",
    tag2: "AI-POWERED LEARNING",
  },
  agriculture: {
    id: "agriculture",
    label: "Agriculture",
...
function resolveTheme(card: VisionCardData): Theme {
  const text = `${card.theme} ${card.technology.join(" ")} ${card.beneficiaries.join(" ")}`.toLowerCase();
  if (/education|school|student|learning/i.test(text)) return THEMES.education;
  if (/agriculture|farm|crop|rural/i.test(text)) return THEMES.agriculture;
  if (/health|hospital|medical|wellness/i.test(text)) return THEMES.healthcare;
  if (/space|science|satellite|isro|rocket/i.test(text)) return THEMES.space;
  if (/environment|climate|renewable|sustainab|energy/i.test(text)) return THEMES.environment;
  if (/women|empower/i.test(text)) return THEMES.empowerment;
  if (/infrastructure|transport|manufactur|smart city|energy/i.test(text)) return THEMES.infrastructure;
  if (/employment|work|job|skill|entrepreneur/i.test(text)) return THEMES.employment;
  if (/ai|artificial|technology|digital|tech|data/i.test(text)) return THEMES.ai;
  return THEMES.technology;
}

function buildTags(card: VisionCardData, theme: Theme): string[] {
  const tags = [theme.tag1, theme.tag2];
  const pool = [...card.technology, ...card.beneficiaries];
  for (const item of pool) {
    if (tags.length >= 3) break;
    const key = Object.keys(TAG_EMOJI).find(
      (known) => item.toLowerCase().includes(known.toLowerCase())
    );
    if (key) {
      const value = `${TAG_EMOJI[key]} ${key.toUpperCase()}`;
...
function quoteFontSize(quote: string): number {
  ...
  if (length <= 48) return 92;
  if (length <= 80) return 78;
  if (length <= 110) return 66;
...
function personaFontSize(name: string): number {
...
function truncate(quote: string, max = 220): string {
...
function buildCaption(
  ...
  if (mode === "challenge") {
    return `I shared my vision for India 2047 🇮🇳 Now it's your turn. Create yours!\n\n${url}\n\n#ViksitBharat2047 #India2047`;
  }
  return `🇮🇳 This is my vision for India 2047. What is yours?\n\n“${quote}”\n\n${tags.join("  ")}\n\nCreate your vision at ${url}\n\n#ViksitBharat2047 #India2047 #AIForIndia`;
}
...
function IndiaSilhouette({ accent, className }: { accent: string; className: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 666.67 777.33"
      aria-hidden="true"
      fill="none"
    >
      <defs>
...
export function VisionCard({ card, name, language, t }: Props) {
  const cardRef = useRef<HTMLDivElement>(null);
  const frameRef = useRef<HTMLDivElement>(null);
  const [scale, setScale] = useState(1);
  const [shareOpen, setShareOpen] = useState<ShareMode | null>(null);
  const [preparing, setPreparing] = useState(false);
  const [shareReady, setShareReady] = useState<ShareReady | null>(null);
  const [shareError, setShareError] = useState("");

  const theme = useMemo(() => resolveTheme(card), [card]);
  const tags = useMemo(() => buildTags(card, theme), [card, theme]);
  const quote = useMemo(
    () => truncate((card.shareableVision ?? card.quote).trim()),
    [card]
  );
  const displayName = name.trim().slice(0, 40);
  const initials = initialsOf(name);
  const personaHeading = t.nameVisionForIndia.replace("{name}", displayName);
  ...
  async function downloadPng() {
    ...
    const slug = displayName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "my";
    link.download = `${slug}-india-2047-vision.png`;
    link.href = dataUrl;
    link.click();
  }

  async function openShare(mode: ShareMode) {
    if (!cardRef.current) return;
    setShareOpen(mode);
    setShareReady(null);
    setShareError("");
    ...
    try {
      ...
      const publicCard = await createPublicCard({
        name: displayName,
        theme: card.theme,
        impact: card.impact,
        quote,
        shareableVision: card.shareableVision ?? card.quote,
        tags,
        language,
        image: dataUrl,
      });
      setShareReady({
        url: publicCard.share_url,
        imageUrl: publicCard.image_url,
        pngBlob,
        caption: buildCaption(mode, quote, tags, publicCard.share_url),
      });
    } catch {
  ...
  return (
    <section className="card-section" aria-labelledby="card-title">
      <div className="section-heading">
        <p>{t.invite}</p>
        <h2 id="card-title">{t.cardTitle}</h2>
      </div>

      <div className="card-frame" ref={frameRef}>
        <div
          className="card-scale"
          style={{ height: Math.round(CARD_H * scale), transform: `scale(${scale})` }}
        >
          <div
            className="vision-card-preview"
            ref={cardRef}
            style={
              {
                width: CARD_W,
          ...
          >
            <div className="card-bg">
              ...
              <div className="card-glow glow-saffron" />
              <div className="card-glow glow-green" />
              <div className="card-glow glow-accent" />
              <div className="card-year-mark" aria-hidden="true">
                2047
              </div>
              <IndiaSilhouette accent={theme.accent} className="card-india" />
              <span className="card-theme-emoji" aria-hidden="true">
                {theme.emoji}
              </span>
            ...
            <div className="card-inner">
              <header className="card-head">
                <div className="card-brand">
                  ...
                  <div className="card-brand-text">
                    <p className="card-brand-name">{t.appName}</p>
                    <p className="card-brand-tagline">{t.tagline}</p>
                  </div>
                </div>
              </header>
              ...
              <div className="card-hero">
                <div className="card-persona">
                  ...
                  <p
                    className="card-persona-line"
                    style={{ fontSize: `${personaFontSize(displayName)}px` }}
                  >
                    {personaHeading}
                  </p>
                </div>
              ...
              <footer className="card-foot">
                <div className="card-tags">
                  {tags.map((tag) => (
                    <span className="card-tag" key={tag} style={{ color: theme.accent }}>
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="card-divider" style={{ background: theme.accent }}>
                  <span className="card-diamond" style={{ background: theme.accent }} />
                </div>
                <p className="card-statement">{t.imagineContributeBuild}</p>
                <div className="card-cta">
                  <span>{t.appName}</span>
                  <span className="card-cta-sep">·</span>
                  <span>{APP_URL}</span>
                </div>
              </footer>
            </div>
      ...
      <div className="share-actions">
        ...
        <button type="button" className="button primary" onClick={() => openShare("share")}>
          <Share2 size={18} /> {t.shareMyVision}
        </button>
        <button type="button" className="button secondary" onClick={() => openShare("challenge")}>
          <Users size={18} /> {t.challengeYourFriends}
        </button>
      </div>
      ...
      {shareOpen && !shareReady ? (
        <div className="share-overlay" role="dialog" aria-modal="true">
          <div className="share-loading">
            ...
            <p>{preparing ? t.preparingCard : ""}</p>
            {shareError ? <p className="share-error">{shareError}</p> : null}
            {shareError ? (
              <button type="button" className="button secondary" onClick={closeShare}>
                Close
              </button>
            ) : null}
          </div>
